        ...
        >
            <div className="p-6 h-full flex flex-col">
                ...
                <div className="shrink-0 border-b-4 border-stone-600 pb-4 mb-4">
                    <h2 className="text-5xl text-yellow-400 font-bold uppercase tracking-widest mb-2 drop-shadow-md">{t('achievements_drawer.title')}</h2>
                    <div className="flex gap-6 text-stone-400 text-2xl">
                        <p>{t('achievements_drawer.total_level')} <span className="text-white font-bold">{totalLevels}</span></p>
                        <p>{t('achievements_drawer.badges')} <span className="text-yellow-400 font-bold">{totalBadges}</span></p>
                        <p>{t('achievements_drawer.achievements')} <span className="text-green-400 font-bold">{unlockedAchievements}/{totalAchievements}</span></p>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 scrollbar-hide">
                                    ...
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-center mb-1">
                                                <h3 className="text-2xl font-bold text-white tracking-wide truncate">{t(`skills.${key}.name`, { defaultValue: key }).toUpperCase()}</h3>
                                                <span className="text-stone-400 text-xl font-bold ml-2 shrink-0">{t('achievements_drawer.lvl')} {userSkill.level}</span>
                                            </div>
                                    ...
                                    <div className="flex flex-wrap gap-2 items-center">
                                        {BADGE_TIERS.map((tier) => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    title={`${t('badges.' + tier.title.toLowerCase())} - Level ${tier.level}`}
                                                >
                                                    ...
                                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-black/90 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                                                        {t('badges.' + tier.title.toLowerCase())} (Lvl {tier.level})
                                                    </div>
                                                </div>
                                            );
                                        })}
                                        ...
                                        {legendaryCount > 0 && (
                                            ...
                                            >
                                                ...
                                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-black/90 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                                                    {t('achievements_drawer.legendary_badge')}
                                                </div>
                                            </div>
                                        )}
                                    </div>
                    ...
                    <div className="mt-8">
                        <h3 className="text-3xl text-yellow-400 font-bold uppercase tracking-widest mb-4 border-b-2 border-stone-600 pb-2">
                            {t('achievements_drawer.special_achievements')}
                        </h3>
                        <div className="grid grid-cols-6 gap-3">
                            {Object.values(ACHIEVEMENTS).map((achievement, index) => {
                                ...
                                let progress = 0;
                                let tierProgress = 0;
                                let displayName = t(`achievements.${achievement.id}.name`, { defaultValue: achievement.name });
                                let tierColor = null;
                                ...
                                if (achievement.isTiered) {
                                    ...
                                    if (unlocked && currentTierIndex >= 0) {
                                        ...
                                        displayName = achievement.tiers[currentTierIndex].tierName;
                                    }
                                }
                                    ...
                                    <div key={achievement.id} className="relative group">
                                        ...
                                        <div className={`absolute z-50 ${tooltipPosition} top-0 w-64 bg-slate-900 border-2 border-yellow-400 rounded-lg shadow-2xl p-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none`}>
                                            ...
                                            {achievement.isTiered && (
                                                <div className="mb-2">
                                                    <div className="flex justify-between text-xs text-slate-400 mb-1">
                                                        <span>{t('achievements_drawer.progress')} {progress}</span>
                                                        {nextTier && <span>{t('achievements_drawer.next')} {nextTier.level}</span>}
                                                    </div>
                                                    {nextTier && (
                                                        <>
                                                            ...
                                                            <p className="text-xs text-slate-500 mt-1">
                                                                {t('achievements_drawer.next')} {t(`tier_names.${currentTierIndex + 1}`, { defaultValue: '' })}
                                                            </p>
                                                        </>
                                                    )}
                                                    {!nextTier && unlocked && (
                                                        <p className="text-xs text-green-400 font-bold">{t('achievements_drawer.max_tier_reached')}</p>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            <div className={`text-xs px-2 py-1 rounded ${unlocked ? 'bg-green-900/30 text-green-400' : 'bg-red-900/30 text-red-400'}`}>
                                                {unlocked ? t('achievements_drawer.unlocked') : t('achievements_drawer.locked')}
                                            </div>
                                        </div>
                                    </div>
                                );
