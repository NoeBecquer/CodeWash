...
const App = () => {
    ...
    // Handle wrong answer - heal boss or damage player
    const handleWrongAnswer = (encounterType) => {
        if (battlingSkillId && encounterType === 'boss') {
    ...
    // Update mobs after defeating encounter
    const updateMobsAfterDefeat = (skillConfig, current, newState) => {
        if (skillConfig.id === 'memory') {
            newState.newMemoryMob = getRandomFriendlyMob();
        }
        if (skillConfig.id === 'patterns') {
            newState.newPatternMob = getRandomMob(current.currentMob);
            newState.newPatternMobAura = getRandomAura();
        }

        // Update stable mobs for combat skills on completion
        const combatSkillMobUpdates = {
            'reading': () => {
                newState.newReadingMob = getRandomMob(current.readingMob);
                newState.newReadingMobAura = getRandomAura();
            },
            'math': () => {
                newState.newMathMob = getRandomMob(current.mathMob);
                newState.newMathMobAura = getRandomAura();
            },
            'writing': () => {
                newState.newWritingMob = getRandomMob(current.writingMob);
                newState.newWritingMobAura = getRandomAura();
            }
        };
        ...
        if (getEncounterType(current.level) === 'miniboss') {
            newState.newMiniboss = getRandomMiniboss();
            newState.newMinibossAura = getRandomAura();
        }

        // Update boss when defeating a boss encounter
        if (getEncounterType(current.level) === 'boss') {
            newState.newBoss = getRandomBoss();
            newState.newBossAura = getRandomAura();
        }
    };

    // Process level restoration and level ups
    const processLevelingLogic = (skillConfig, newState) => {
        if (newState.newLostLevel) {
            newState.newLevel += 1;
            newState.newLostLevel = false;
            newState.newRecoveryDifficulty = null;
            setShowLevelRestored(true);
            setTimeout(() => setShowLevelRestored(false), 2000);
            playNotification();
        }

        const xpToLevel = calculateXPToLevel(newState.newDifficulty, newState.newLevel);
        if (newState.newXp >= xpToLevel) {
            processMultipleLevelUps(skillConfig, newState, xpToLevel);
        }
    };

    const loopOvernewState = (oldLevel, newState, skillConfig) => {

        for (let lvl = oldLevel; lvl < newState.newLevel; lvl++) {
            if (lvl % 20 === 0 && lvl > 0) {
                // Boss at level lvl was just defeated
                const newTier = Math.floor(lvl / 20);
                if (newState.newDifficulty < 7) {
                    newState.newDifficulty++;
                }
                // Award badge for this tier if not already earned
                if (!newState.newBadges.includes(newTier) && newTier <= 7) {
                    newState.newBadges.push(newTier);
    ...
    // Process multiple level ups from single hit
    const processMultipleLevelUps = (skillConfig, newState, xpToLevel) => {
        const levelsGained = Math.floor(newState.newXp / xpToLevel);
        const oldLevel = newState.newLevel;
        newState.newLevel += levelsGained;
        newState.newXp = newState.newXp % xpToLevel;

        // Only process boss progression if not cleaning skill
        if (skillConfig.id !== 'cleaning') {
            loopOvernewState(oldLevel, newState, skillConfig);
        // Get new mob if not at boss level
        if (newState.newLevel % 20 !== 0 && (newState.newLevel - 1) % 20 !== 0) {
            newState.newMob = getRandomMob(newState.newMob);
        }
    ...
    const handleSuccessHit = (skillId, isWrong) => {
        ...
        if (isWrong === 'WRONG') {
            if (battlingSkillId) {
                const encounterType = getEncounterType(skills[battlingSkillId].level);
                handleWrongAnswer(encounterType);
            }
            return;
        }
        ...
        setSkills(prev => {
            ...
            const newState = {
                newMobHealth: current.mobHealth - actualDamage,
                newLevel: current.level,
                newXp: current.xp,
                newMob: current.currentMob,
                newDifficulty: current.difficulty || 1,
                newBadges: [...(current.earnedBadges || [])],
                newMobMaxHealth: current.mobMaxHealth,
                newLostLevel: current.lostLevel,
                newRecoveryDifficulty: current.recoveryDifficulty,
                newMemoryMob: current.memoryMob,
                newPatternMob: current.patternMob,
                newMiniboss: current.currentMiniboss,
                newBoss: current.currentBoss,
                newReadingMob: current.readingMob,
                newMathMob: current.mathMob,
                newWritingMob: current.writingMob,
                newReadingMobAura: current.readingMobAura,
                newMathMobAura: current.mathMobAura,
                newWritingMobAura: current.writingMobAura,
                newPatternMobAura: current.patternMobAura,
                newMinibossAura: current.currentMinibossAura,
                newBossAura: current.currentBossAura
            };
            ...
            if (newState.newMobHealth > 0) {
                newState.newXp += xpPerHit;
            }
            
            // Mob defeated!
            if (newState.newMobHealth <= 0) {
                // Calculate remaining XP to award
                ...
                setStats(prevStats => {
                    ...
                    setTimeout(() => {
                        checkAchievements(prevStats, newStats, prev, { ...prev, [skillId]: { ...current, level: newState.newLevel } });
                    }, 100);
                    
                    return newStats;
                });
                
                // Update mobs after defeat
                updateMobsAfterDefeat(skillConfig, current, newState);
                
                // Process leveling logic
                processLevelingLogic(skillConfig, newState);
                
                // Spawn new mob with fresh health
                newState.newMobMaxHealth = calculateMobHealth(newState.newDifficulty);
                newState.newMobHealth = newState.newMobMaxHealth;
                if (newState.newLevel % 20 !== 0) {
                    newState.newMob = getRandomMob(current.currentMob);
                }
            }
            ...
            return {
                ...
                [skillId]: {
                    ...
                    xp: newState.newXp,
                    currentMob: newState.newMob,
                    difficulty: newState.newDifficulty,
                    earnedBadges: newState.newBadges,
                    mobHealth: newState.newMobHealth,
                    mobMaxHealth: newState.newMobMaxHealth,
                    lostLevel: newState.newLostLevel,
                    recoveryDifficulty: newState.newRecoveryDifficulty,
                    memoryMob: newState.newMemoryMob,
                    patternMob: newState.newPatternMob,
                    currentMiniboss: newState.newMiniboss,
                    currentBoss: newState.newBoss,
                    readingMob: newState.newReadingMob,
                    mathMob: newState.newMathMob,
                    writingMob: newState.newWritingMob,
                    readingMobAura: newState.newReadingMobAura,
                    mathMobAura: newState.newMathMobAura,
                    writingMobAura: newState.newWritingMobAura,
                    patternMobAura: newState.newPatternMobAura,
                    currentMinibossAura: newState.newMinibossAura,
                    currentBossAura: newState.newBossAura
                }
            };
        });
